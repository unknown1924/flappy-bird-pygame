...
def welcomeScreen():
	"""
	shows welcome image when starting the game
	"""
	#playerx = int(SCREENWIDTH/5)
	playerx = int(72)
	playery = int(72)
	#playery = int(SCREENHEIGHT - GAME_SPRITES['player'].get_height())/2
	messagex = int(SCREENWIDTH +50 - GAME_SPRITES['message'].get_width())/2
...
def maingame():
	...
	while True:
		...
		crashTest = isCollide(playerx, playery, upperPipes, lowerPipes) #will return true if player crashed
		if crashTest:
			return
		...
		#Add a new pipe when the first pipe about to go to the left
...
def getRandomPipe():
	"""
	generate position fo two pipes for blitting on the screen 
	"""
...
while True:
	welcomeScreen() #shows welcome screen untill a keypress
